...
See `@.agentic/shared/skills/gates/open_questions_gate.md` (Affects: `product_backlog.md`).

## Instructions
1. Review all artifacts.
2. Ask via `@.agentic/shared/skills/interaction/questions_format.md`.
3. Check for missing capabilities.
4. Validate priority assumptions.

If anything is blocked by missing information, add an unchecked `[Blocking]` item under `## Open` in `.agentic/artifacts/open_questions.md` and stop.

If producing the backlog requires making new decisions, stop and capture them as decision log row(s) in `.agentic/artifacts/decision_log.md` first.
...
> (It stays a living document during implementation.) If yes, tag **@.agentic/implementation/01_pick_feature.md**.”
